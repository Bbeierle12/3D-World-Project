...
import { GaitType, MovementMode } from '../controller/MovementModes.js';
...
export class ProceduralAnimation {
   ...
   * @param {string} gait
   * @param {number} cyclePhase - Walk cycle phase (0-1)
   * @param {number} time - Elapsed time
   * @param {number} deltaTime
   */
  update(movementMode, gait, cyclePhase, time, deltaTime) {
  ...
  calculateTargets(movementMode, gait, phase, time) {
    let torsoLean = 0;
    let torsoTwist = 0;
    let headBob = 0;
    let hipSway = 0;
